...
function imageAsDataUrl(source) {
    ...
    const imageUrl = `${originalUrl}${separator}w=320&h=320&fit=fill&fm=jpg&q=78&bg=ffffff`;

    return fetch(imageUrl, { signal: AbortSignal.timeout(15000) })
        .then((response) => {
...
export async function createCatalogWorkbook(categories, { selectedOnly = false } = {}) {
    ...
    categories.forEach((category) => {
        ...
        worksheet.getCell("A1").value = selectedOnly ? "MARBOK – PONUDA IZABRANIH PROIZVODA" : "MARBOK – KATALOG PROIZVODA";
